	...
	while(1)
	{
		...
		for(i = 0; i < JOBSIZE; i++)
		{
			...
			if(job_list[i] != NULL && job_list[i]->state == STATE_RUNNING)
			{
				...
				//设置监视任务
				if(job_list[i]->fsm12->state == STATE_R)
					job_list[i]->pfd[0].events |= POLLIN;
				if(job_list[i]->fsm21->state == STATE_W)
					job_list[i]->pfd[0].events |= POLLOUT;

				if(job_list[i]->fsm12->state == STATE_W)
					job_list[i]->pfd[1].events |= POLLOUT;
				if(job_list[i]->fsm21->state == STATE_R)
					job_list[i]->pfd[1].events |= POLLIN;

				//监视
				while(poll(job_list[i]->pfd, PFDSIZE, 0) < 0)
				{
					if(errno == EINTR)
						continue;
					perror("poll()");
				...
				//查看监视结果
				if(job_list[i]->pfd[0].revents & POLLIN || job_list[i]->pfd[1].revents & POLLOUT)
					rel_fsm_driver(job_list[i]->fsm12);
				if(job_list[i]->pfd[1].revents & POLLIN || job_list[i]->pfd[0].revents & POLLOUT)
...
int rel_add_job(int fd1, int fd2)
{
	...
	job->pfd[0].fd = job->fd1;
	job->pfd[0].events = 0;
	job->pfd[1].fd = job->fd2;
	job->pfd[1].events = 0;
